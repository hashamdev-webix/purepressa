...
const stepLabels = ["Choose Pack Size", "Select Products", "Order Type"];
const builderProducts = products.filter((product) => product.category !== "packs");

export const BuildYourPack = () => {
  const navigate = useNavigate();
  const { addItem, openCart } = useCart();
  const [currentStep, setCurrentStep] = useState(1);
  const [packSizeValue, setPackSizeValue] = useState(packSizes[0].value);
  const [quantities, setQuantities] = useState({});
  const [orderTypeValue, setOrderTypeValue] = useState(orderTypes[0].value);

  const selectedPack = packSizes.find(
    (packSize) => packSize.value === packSizeValue,
  );
  const selectedOrderType = orderTypes.find(
    (orderType) => orderType.value === orderTypeValue,
  );
  ...
  const createCartItem = () => {
    ...
    return {
      id: `byo-${Date.now()}`,
      name: `Build Your Own Pack (${targetCount}x)`,
      price: subtotal,
      image: null,
      meta: `${selectedOrderType.label} · ${contentsSummary}`,
    };
  };
      ...
      <Container>
        ...
        <div className="mt-10 grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="rounded-lg border border-border bg-surface p-5 shadow-soft md:p-8">
            {currentStep === 1 && (
              <fieldset>
                ...
                <p className="mt-2 text-body">
                  Select the pack format that fits your routine.
                </p>

                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  {packSizes.map((packSize) => {
                    const inputId = `pack-size-${packSize.value}`;
              ...
              <div>
                ...
                <div className="mt-6" aria-live="polite">
                  ...
                  <div
                    ...
                    aria-label="Pack selection progress"
                  >
                    <div
            ...
            {currentStep === 3 && (
              <fieldset>
                ...
                <p className="mt-2 text-body">
                  Order once or keep your custom pack on a recurring schedule.
                </p>

                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  {orderTypes.map((orderType) => {
                    const inputId = `order-type-${orderType.value}`;
                    const isSelected = orderTypeValue === orderType.value;
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          checked={isSelected}
                          onChange={() => setOrderTypeValue(orderType.value)}
                          className="sr-only"
                        />
          ...
          <aside
            ...
            aria-label="Custom pack summary"
          >
            ...
            <dl className="mt-6 space-y-4 border-y border-border py-5">
              <div className="flex items-start justify-between gap-4">
                <dt className="text-sm text-muted">Pack size</dt>
                <dd className="text-right text-sm font-semibold text-ink">
                  {selectedPack.label}
                </dd>
              </div>
              <div className="flex items-start justify-between gap-4">
                <dt className="text-sm text-muted">Order type</dt>
                <dd className="text-right text-sm font-semibold text-ink">
            ...
            <div className="mt-5">
              ...
              {selectedProducts.length > 0 ? (
                ...
                <p className="mt-2 text-sm text-muted">
                  Select products to build your pack.
                </p>
              )}
            </div>
