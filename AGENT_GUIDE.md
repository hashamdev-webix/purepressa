...
- Decisions / deviations from spec: Used custom-styled radio buttons with appearance-none instead of native browser radio styling to achieve the exact filled-primary vs empty-grey visual distinction required. Added subtle ring-1 ring-primary/20 to selected tier rows for additional clarity beyond just the border change.
- Known issues: None introduced by this change.
- Next steps: Continue with feature work or backend integration.

### 2026-06-24 — Kiro / Claude Sonnet 4.5 (Subscription Plan Card Redesign)

- Done: Redesigned subscription plan cards on the Subscriptions page for a more premium, polished UX. Removed the large cream placeholder image block entirely from SubscriptionPlanCard component and created image-less, content-first pricing cards with clean lucide icon frequency pills. New card structure: frequency pill with dynamic icon mapping (CalendarDays for simple frequencies, Repeat for multi-option frequencies, Building2 for business/office plans) at top, "Most Popular" badge when applicable, plan name, description, "Best for" line with subtle border divider, and pricing area pinned to bottom via mt-auto. Most Popular plan (Weekly Wellness) now has a distinct 2px primary ring. All cards have equal height via grid items-stretch and h-full wrapper divs. Tiered plans show two full-width buttons (Add to Cart outline + Order Now primary) in flex row. Office plan shows custom pricing text and single Contact Us button. Build Your Own shows large "From $44.99" pricing and single Build button. Updated grid in SubscriptionPlans section to ensure equal-height stretching. All existing onClick/cart/navigation logic preserved exactly. Removed unused MediaPlaceholder and Package icon imports. Verified npm run lint and npm run build pass clean, dev server runs without errors.
